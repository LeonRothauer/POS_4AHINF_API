...
function updateNoteInAPI(noteId, title, description) {
  const updatedNote = {
    ...
    titel: title,
    notiz: description,
    erstelldatum: getNoteCreationDate(noteId) // Erstelldatum abrufen und übergeben
  };
...
function getNoteCreationDate(noteId) {
  const listItem = document.querySelector(`li[data-id="${noteId}"]`);
  const dateElement = listItem.querySelector(".date");
